...
export const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    // Get current order with additional information
    const orderResult = await client.query(`
      SELECT o.*, 
    ...
    // If order is ready, automatically notify all available drivers
    if (status === 'ready') {
      // Récupérer les informations de la commande
      const orderDetailsResult = await client.query(`
        SELECT o.*, p.owner_id as pharmacy_owner_id, p.name as pharmacy_name,
      ...
      const availableDriversResult = await client.query(`
        ...
        AND (dl.is_available IS NULL OR dl.is_available = true)
      `);
      
      console.log(`🔔 Notification de ${availableDriversResult.rows.length} livreurs pour la commande ${orderId}`);
